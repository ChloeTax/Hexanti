...
core.register_on_player_receive_fields(function(player, formname, fields)
    if formname ~= "hexcasting:hexInput" then
        return
    end
...
function trinket(itemstack, user, pointed_thing)
    ...
    if user:get_player_control().sneak then
        ...
        core.show_formspec(user:get_player_name(), "hexcasting:hexInput", table.concat(formspec, ""))
...
core.register_craftitem("hexcasting:trinket", {
    description = "casts a stored hex",
    inventory_image = "hexcasting_trinket.png",
    stack_max = 1,
    on_use = trinket,
})
